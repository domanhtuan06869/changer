...
let androidVersion = JSON.parse(
  fs.readFileSync(`${__dirname}/wininfo/sdk.json`)
);
let deviceInfos = JSON.parse(
  fs.readFileSync(`${__dirname}/wininfo/x_devices.json`)
);
let networks_client = JSON.parse(
  fs.readFileSync(`${__dirname}/wininfo/network_client.json`)
);
...
function getNetwork(countryIso, operator) {
  ...
  let byCountry = networks.filter(
    (x) => x.NetworkCountryIso === countryIso.toUpperCase()
  );
...
function randomDate(utc) {
  ...
  return dateFormat(new Date(utc), "dddd mmmm d HH:MM:ss _pp_ yyyy")
    .toString()
    .replace("_pp_", zone);
}
...
app.get("/networks", (req, res) => {
  res.send({ networks_client: networks_client });
});
...
app.get("/change", (req, res) => {
  (async () => {
    try {
      ...
      props = props.concat(
        gen4Props("ro.product.*.manufacturer", manufacturer)
      );
      ...
      props = props.concat(
        gen4Props("ro.*.build.version.incremental", buildIncremental)
      );
      ...
      sim["SimSerial"] = `89${isoCode}${operator.substring(
        3,
        5
      )}${getRndInteger(15, 20)}0${getRndInteger(1, 9)}${randomNumber(9)}`;
